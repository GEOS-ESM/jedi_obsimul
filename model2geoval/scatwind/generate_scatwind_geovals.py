...
def generate_scatwind_geovals(sensor, year, month, day, analtime, analtimep3):
    """
...
#5 2D variables
    ...
    geoval_ds['geometric_height'][:,0] = geoval_ds['geopotential_height'][:,0] * 6371000.0 \
                                            / (6371000.0 - geoval_ds['geopotential_height'][:,0]) 
    for k in range(1,nlevs):
       ...
       geoval_ds['geometric_height'][:,k] = geoval_ds['geopotential_height'][:,k] * 6371000.0 \
                                            / (6371000.0 - geoval_ds['geopotential_height'][:,k]) 
