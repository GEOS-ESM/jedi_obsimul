...
class RADIANCE(GEOVAL):
    ...
    def populate(self, nr, sfc )
        """
        Create GEOVAL object for radiances. On input,

        nr  --- xr.Dataset, sampled lature
        sfc --- xr.Dataset, GFS gridded surface types
        ...
        if np.any(np.abs(tot_frac.values-1)>1e-3):
        ...
        # Define necessary lookup lists and constants for soil and vegetation types
        ...
        # Sample GFS vegetation/soil type at obs location
        stype = sfc.stype.interp(lon=nr.lon,lat=nr.lat,method='nearest')
        vtype = sfc.vtype.interp(lon=nr.lon,lat=nr.lat,method='nearest')
        vfrac = sfc.vfrac.interp(lon=nr.lon,lat=nr.lat,method='nearest')

        self.land_type_index_NPOESS = igbp_to_gfs[vtype.values.astype('int')-1]  # double check this
        self.vegetation_type_index = igbp_to_gfs[vtype.values.astype('int')-1]
        self.soil_type = map_soil_to_crtm[stype.values.astype('int')-1]  
        self.vegetation_area_fraction[:] = vfrac
        ...
        self.surface_geopotential_height = nr.PHIS / constants.MAPL_GRAV  # phis[m2 sec-2] --> geo.pot.height[m]
        self.surface_geometric_height = self.surface_geopotential_height * 6371000.0 / (6371000.0 - self.surface_geopotential_heigh)
        self.height_above_mean_sea_level_at_surface = self.surface_geopotential_height * 6371000.0 / (6371000.0 - self.surface_geopotential_height)
        ...
        self.mole_fraction_of_ozone_in_air[:] = nr.O3  # Temporary value
...
#   --------------------------
    ...
    # ---------------------------
    gv = RADIANCE(nr.time, nr.lat, nr.lon, nr.PLE )
