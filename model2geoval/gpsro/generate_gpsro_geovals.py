...
def generate_gpsro_geovals(sensor, year, month, day, analtime, analtimep3):
    """
...
#5 2D variables
    ...
    for k in range(nlevs):
       geoval_ds['geopotential_height_levels'][:, k+1] = geoval_ds['geopotential_height_levels'][:, k] \
                          + 29.3 * virtual_temp[:, k] * np.log(air_pressure_levels[:, k] / geoval_ds['air_pressure_levels'][:, k+1]) 
